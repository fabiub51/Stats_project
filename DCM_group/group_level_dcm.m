...
for sub = 1:10
    clear matlabbatch
    data_path = sprintf('%s/sub-%03d/conc_runs_IMG_STIM', data_dir, sub);
    cd(data_path);
    model_output= sprintf('/Users/almila/Desktop/stats project copy 4/groupleveldcm/sub-%03d', sub);
    % Load the SPM model
    ...
    % Save the model
    modelfile1_name= sprintf('sub-%03d_DCM_model_simplified_IMG.mat', sub);
    ...
    % Save the model
    modelfile2_name= sprintf('sub-%03d_DCM_model_alternative_IMG_ff.mat', sub);
    ...
    % Save the model
    modelfile3_name= sprintf('sub-%03d_DCM_model_alternative_IMG_fb.mat', sub);

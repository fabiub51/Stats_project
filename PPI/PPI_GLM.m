%% Specifying and estimating a first-level GLM for each subject
clear matlabbatch
...
for i = 1:length(sub_folders)
    ...
    % Adding the onsets for every condition 
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess(i).cond(7).duration = 3; 
...
% Estimation was done manually for each subject using the GUI
